<template>
    <li v-if="menuItems?.items?.length > 0" class="mb-4">
        <p class="text-sm font-semibold mb-3">{{ menuItems?.sectionName }}</p>
        <ul v-for="(parentMenu, index) in menuItems?.items" :key="index" class="">
            <li v-if="parentMenu?.items?.length > 0">
                <a v-on:click="showMenu($event, parentMenu)" type="button"
                    ...
                    <div
                        class="flex items-center justify-between gap-3 px-3 py-2 rounded-sm hover:text-primaryOrange duration-500 transition-all">
            ...
            <li v-else>
                <router-link
                    :to="parentMenu?.path ?? '#'"
                    :class="isMenuActive(parentMenu) ? 'text-primaryOrange flex items-center gap-2 text-sm px-3 py-2 hover:text-primaryOrange duration-500 transition-all' : 'flex items-center gap-4 text-sm px-3 py-2 hover:text-primaryOrange duration-500 transition-all'">
                    <span v-html="parentMenu?.icon" class="flex items-center"></span>
                    <span href=""> {{ parentMenu?.name }}</span>
                </router-link>
            </li>
